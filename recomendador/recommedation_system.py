...
df_reviews = pd.read_csv(args['file']).drop_duplicates(subset=['Tripadvisor'],keep='first')
...
selection = user_selection(args["selection"])

results = best_results(dict_user(df_reviews,selection),df_reviews,args["pois"],relevant=args["selection"])

if args['output'] != "default":
   results.to_csv("{output}_{sufix}.csv".format(output=args["output"],sufix=args["selection"]),index=False)
